...
// Findet das erste vollständige, balancierte JSON (Array oder Objekt) im Text.
// Berücksichtigt String-Literale, damit Klammern in Strings nicht mitzählen.
...
function extractJSON(text) {
  // 1. Versuch: explizit markierter ```json ... ``` Block
  const fenced = text.match(/```json\s*([\s\S]*?)```/);
  if (fenced) {
    try { return JSON.parse(fenced[1].trim()); } catch (e) { /* weiter unten */ }
  }

  // 2. Versuch: balanciertes Array oder Objekt suchen — was zuerst kommt
  const arrStart = text.indexOf("[");
  const objStart = text.indexOf("{");
  let candidate = null;
  if (arrStart !== -1 && (objStart === -1 || arrStart < objStart)) {
    candidate = findBalancedJSON(text, "[", "]");
    if (!candidate) candidate = findBalancedJSON(text, "{", "}");
  } else if (objStart !== -1) {
    candidate = findBalancedJSON(text, "{", "}");
    if (!candidate) candidate = findBalancedJSON(text, "[", "]");
  }
  if (candidate) {
    try { return JSON.parse(candidate); } catch (e) {
      throw new Error("JSON nicht parsbar: " + e.message);
    }
  }
